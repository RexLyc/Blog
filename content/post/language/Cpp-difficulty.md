...
4. SGI STL的内存配置器实现：提供了一个更高效率的alloc，它并不属于C++标准，SGI对其进行了封装，其核心特性如下
    ...
    2. 二级配置器的实现核心
        ...
        > 流程可参考[STL-空间配置器分析（内存的配置和释放）](https://blog.csdn.net/sssssuuuuu666/article/details/79245131)，博主绘制了流程图
5. 其他内存工具
...
1. 右值引用出现的原因：
    ...
    - 但当C++拥有构造和析构之后，问题变得复杂了起来。如果函数返回值依然能够放在一个寄存器内则还好。如果是较长的非平凡的结构体、类，则创建这个临时变量的过程，是会调用构造，形成一个真正完整的变量，而赋值时又会调用复制构造，最后这个临时变量还会进行析构。除了性能问题外，还有一些需要讨论的：
        ...
        > 注一：字面值除了字符串字面值以外，都是纯右值，字符串字面值是一个左值（存储在程序的固定段中），类型是```const char[]```，注意虽然可以隐式的转换为```const char*```，但这本质上并不是同一种类型。很明显对二者调用```sizeof```会有不同的结果。
        > 注二：类型转换过程中会产生一个右值。这一点可能被忽略，但当你尝试对一个左值引用赋值一个需做类型转换的对象时，会编译失败。这一点更常见于隐式类型转换发生时。
...
1. 右值引用的目标：
    ...
    > 右值引用**本身是一个左值**，它不仅有名字，实际上仍然可以在等号左边被继续赋值。但要记得它存储的内容是一个右值。
1. “万能引用”：
