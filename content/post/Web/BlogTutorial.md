...
## 写在前面
> 本段文字更新于2025/05/19
因为之前的服务器到期，续费又实在太贵，因此决定使用github page，而且发现github page是支持自定义域名的。而且使用下来发现，图片的加载速度也很快，甚至CDN、图床都没有必要买。直接迁移就可以。

注意几点：
1. 找到合适的hugo.yml，作为github action，并修改其中的hugo版本，以及注意其使用的```hugo build```指令，尤其是baseUrl，如果不正确，会导致图片404
2. 不建议使用顶级域名。无论如何，都需要在你的DNS提供商那里，为你的自定义域名，添加一个CNAME，指向你的```xxx.github.io.```，注意最后有一个点。另外还可以做一下verify提高安全性，位置在**个人设置**->**Setting**->**Pages**->**Add domain**
