...
### 堆
堆是满足当前节点大于（或者小于）子树所有节点这种性质的树。在具体使用中，还会进一步进行变种。下面是一些简单的变种：
1. 二叉堆：一棵完全二叉树，而且是堆
#### 可合并堆
1. [配对堆](https://oi-wiki.org/ds/pairing-heap/)：有如下特点
    - 满足堆性质的多叉树
    - 和常规树不同，通常用兄弟节点链表+子节点指针来表示。跟进一步的，可以使用双向兄弟节点链表，且打头的
    - 合并：如果是大顶堆，时直接将两个根中较大的作为根，另一个变成其子节点。$O(1)$
    - 插入：相当于合并一个单节点堆，且最后加入的子节点，在兄弟节点链表的首位（这样才能$O(1)$）。
    - 查询最大/最小：显然$O(1)$
    - 删除最大/最小：均摊$O(\log{n})$。删除堆顶后，将子节点两两合并，再从尾向首（从旧到新插入）递归两两合并。这时堆整体上大概是右偏的。
    - 增大/减小一个元素的值：最差情况下，需要将节点及其子树从树上切下来，并将其重新合并到堆。
2. 左偏树（Leftist Tree）：
