...
4. 坑：
    1. 二进制数据不建议直接填充到文本类型的数据体中，由于编码问题，不能保证所有的二进制数据得到正确编码（可能被转为一个问号?），对于需要填入到json格式内的二进制数据，可以考虑转为BASE64编码。
5. 大版本的主要变化
    - 0.9：已过时废弃，该版本在协议报头的行中没有版本，只接受GET方法
    - 1.0（1996）：指定了版本号的正式版本，增加了POST/HEAD方法，支持多媒体等丰富的数据类型、多字符集支持、multi-part发送、权限、缓存、返回码、增加了请求头
    - 1.1（1997）：默认采用持久连接（keep-alive）、增加了对缓存、TCP连接复用、管道pipeline机制（一次发多个HTTP请求）、增加了更多方法PUT/PATCH/OPTIONS/DELETE。仍然有HTTP请求队头堵塞问题。
    - 2（2015）：完整的二进制支持（头部和数据体）、多工和数据流（multiplex，客户端和浏览器都可以同时发送多个请求或回应，而且不用按照顺序一一对应）、server push、头信息压缩
    - 3（标准仍在开发中）：弃用TCP协议，改为使用基于UDP协议的QUIC协议实现
6. 流水线
    - 非流水线模式
    - 流水线模式
