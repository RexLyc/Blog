...
- 层次式架构设计
  ...
  - 架构案例分析
    ...
    - 基于物联网架构的电子小票服务系统
- 云原生架构设计
  - 云原生架构是基于云原生技术的一组架构原则和设计模式的集合，目标是将云应用中的非业务代码部分进行**最大化的剥离**，让云设施接管应用中的非功能特性（弹性、韧性、安全、可观测性、灰度等），让业务不再担心非功能性业务导致的终端，具备轻量、敏捷、高度自动化的特点。
  ...
  - 相关技术：
    - 容器技术：
      ...
      - 充分发挥云计算弹性又是，降低运维成本
      - K8s已成为容器编排的实施标准：资源调度、应用部署、自动修复、服务发现和负载均衡、弹性伸缩、声明式API、可扩展架构、可移植性
    - 云原生微服务：
...
- 嵌入式系统架构设计
  ...
  - 嵌入式中间件（不是Web意义上的中间件）
    - 中间件：平台 + 通信。对嵌入式应用屏蔽底层操作系统的异构型
    - 功能：网络通信、内存管理、数据处理。
    - 特点：通用性、异构型、分不行、协议规范性、接口标准化
...
- 通信系统架构设计
  - 网络形式：局域网、广域网、移动通信网
    - 局域网
      ...
      - 环形架构：多态核心交换设备连接成双RPR动态弹性分组环，构建网络的核心。投资高、实现难度较高。
  ...
  - 架构案例分析
    ...
    - 5G网络应用：IoT
- 安全架构设计
  - 信息系统威胁：物理化境、通信链路、网络系统、操作系统、应用系统、管理系统
...
- **大数据架构设计**
  ...
  - 架构特征：鲁棒性和容错性、低延迟读取和更新能力、横向扩容、通用性、延展性、即席查询能力、最少维护能力、可调试性
  - Lambda架构：
